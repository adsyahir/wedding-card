 ...
 */
export function GallerySettings({ images }: { images: GalleryImageForSettings[] }) {
  const router = useRouter();
  const { run: runDelete, pending: deletePending, error: deleteError } = useAdminAction();
  const { run: runReorder, pending: reorderPending, error: reorderError } = useAdminAction();
  ...
  async function handleSaveAlt(id: string) {
    ...
    if (!alt) {
      setAltError("Teks alt tidak boleh kosong.");
      return;
    }
    setAltPendingId(id);
    setAltError(null);
    try {
      ...
      if (!response.ok || !data?.ok) {
        setAltError((data && !data.ok && data.error) || "Ralat tidak dijangka. Sila cuba lagi.");
        return;
      }
      router.refresh();
    } catch {
      setAltError("Ralat rangkaian. Sila cuba lagi.");
    } finally {
  ...
  async function handleDelete(id: string) {
    if (!window.confirm("Padam gambar ini? Tindakan ini tidak boleh dibatalkan.")) return;
  ...
  async function handleUpload(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (remaining <= 0) {
      setUploadError("Galeri sudah penuh. Padam gambar sedia ada sebelum memuat naik lagi.");
      return;
    }

    const file = fileInputRef.current?.files?.[0];
    if (!file) {
      setUploadError("Sila pilih fail gambar.");
      return;
    }
    ...
    if (!allowedTypes.has(file.type)) {
      setUploadError("Format fail tidak disokong. Gunakan JPEG, PNG atau WebP.");
      return;
    }
    if (file.size > MAX_IMAGE_BYTES) {
      setUploadError("Saiz fail terlalu besar (had 5 MB).");
      return;
    }
  ...
  return (
    <section className="flex flex-col gap-4 rounded-xl border border-tan/30 bg-sand/40 p-4">
      <h2 className="font-serif text-xl text-brown-deep">Galeri</h2>

      {images.length === 0 ? (
        <p className="text-xs text-brown/60">
          Belum ada gambar dimuat naik. Kad akan memaparkan gambar placeholder lalai.
        </p>
      ) : (
      ...
      <form onSubmit={handleUpload} className="flex flex-col gap-2 border-t border-tan/30 pt-4">
        <p className="text-sm font-medium text-brown-deep">
          Muat naik gambar baharu ({remaining} baki daripada {MAX_GALLERY_IMAGES})
        </p>
